...
public class SmartPhone {
  ...
  public boolean isInside(String check) { // does app already exist
    for (int i = 0; i < this.getApplication().length; i++) {
      String s = this.getApplication()[i];
      if (s != null) {
        if (s.equals(check)) return true;
      }
    }
  ...
  public void addApplication(String newApplication) {
    if (this.getCurrent() == 100) return;
    if (isInside(this.getApplication(), newApplication)) return;
